    ...
    for i in range(1):
        ...
        for falsecount in range(2):
            ...
            if msg == Et.EnglishAnswer():
                Kkangtong("와아 맞았다능 천재라능")
                score +=1
                break
            elif falsecount < 1:
                Kkangtong("바보라능 그것도 모른다능 다시 풀어보라능")
                falsecount += 1
                continue
            else:
                Kkangtong(" 아까웠다능 답은 {} 이라능".format(Et.EnglishAnswer()))
                break

            if msg == '그만':
                Kkangtong('퀴즈는 여기까지 라능, 아쉽다능')
                break
            base.mainloop()
    Kkangtong("채점시간 이라능")
    Kkangtong("%2d월%2d일 %02d시%02d분 오늘의 영어점수는 %d 이라능"%(now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min,score*10))
...
def Kkangtong(msg):
    ...
    label = tkinter.Label(ChatLog, text='깡통',image=image) 
...
def Baqui(msg):
    ...
    label1 = tkinter.Label(ChatLog, text='바퀴', image=image1)
    label1.image = image1
